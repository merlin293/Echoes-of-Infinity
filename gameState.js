...
const gameState = {
    ...
    currentTierIndex: 0, // Výchozí hodnota při definici objektu
...
function initializeDefaultGameStateVariables() {
    console.log("INIT_DEFAULT_GAME_STATE: Začátek. Aktuální currentTierIndex:", gameState.currentTierIndex);

    // Reset všech vlastností gameState na jejich výchozí hodnoty
    gameState.gold = 0;
    ...
    gameState.currentTierIndex = 0; // Explicitní nastavení na 0
    ...
    // xpToNextLevel se nastaví později, pokud je funkce dostupná
    gameState.talentPoints = 0;
    gameState.clicksSinceLastGuaranteedCrit = 0;
    
    // gameState.equipment se inicializuje pomocí initializeEquipment() níže
    ...
    gameState.gameSettings = {
        showDamageNumbers: true, showGoldAnimations: true,
        soundVolume: 0.5, soundMuted: false   
    };
    ...
    // Reset úrovní talentů (pokud jsou talenty definovány)
    ...
    // Inicializace struktury vybavení
    if (typeof initializeEquipment === 'function') { 
        initializeEquipment(); 
        console.log("INIT_DEFAULT_GAME_STATE: initializeEquipment() zavoláno. gameState.equipment:", JSON.stringify(gameState.equipment));
    } else {
        console.error("INIT_DEFAULT_GAME_STATE: initializeEquipment function not found! Vybavení nemusí být správně inicializováno.");
        gameState.equipment = {}; // Fallback
    ...
    // Inicializace výzkumu a esencí (pokud jsou definice dostupné)
    ...
    // Inicializace dovedností společníků
    if (typeof allCompanions !== 'undefined') {
        for (const companionId in allCompanions) {
            if (allCompanions.hasOwnProperty(companionId)) {
                gameState.companionSkillLevels[companionId] = {}; // Zajistíme existenci objektu pro společníka
    ...
    // Nastavení xpToNextLevel až po nastavení playerLevel
    if (typeof calculateXpToNextLevel === 'function') {
        gameState.xpToNextLevel = calculateXpToNextLevel();
    } else {
        gameState.xpToNextLevel = 70; // Fallback
    }

    console.log("INIT_DEFAULT_GAME_STATE: Dokončeno. Finální currentTierIndex:", gameState.currentTierIndex);
}