...
const MAX_OFFLINE_TIME_SECONDS = 60 * 60 * 8; // 8 hodin
const OFFLINE_GOLD_EARN_PERCENTAGE = 0.25;
const OFFLINE_XP_EARN_PERCENTAGE = 0.10;
const MIN_OFFLINE_TIME_FOR_PROGRESS_SECONDS = 60 * 2; // 2 minuty
...
const talents = {
    // Základní větev
    increasedClickDamage: { name: "Základní Síla", description: "Zvyšuje základní poškození kliknutím o 10% za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 1 + level, effectType: 'base_click_damage_multiplier_percent', effectValue: 0.10, branch: 'basic', icon: '💪' },
    goldVeins: { name: "Zlaté Žíly", description: "Zvyšuje veškeré získané zlato o 3% za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 1 + Math.floor(level/2), effectType: 'gold_multiplier_all_percent', effectValue: 0.03, branch: 'basic', requires: 'increasedClickDamage', icon: '💰' },
    echoAffinity: { name: "Echo Příbuznost", description: "Zvyšuje množství získaných Echo Úlomků z Echa o 5% za úroveň.", maxLevel: 5, currentLevel:0, cost: (level) => 2 + level, effectType: 'echo_shard_multiplier_percent', effectValue: 0.05, branch: 'basic', requires: 'goldVeins', icon: '🔮' },

    // Kritická větev
    critChanceBoost: { name: "Ostříží Zrak", description: "Zvyšuje šanci na kritický zásah o 0.5% za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 2 + level, effectType: 'crit_chance_bonus_percent', effectValue: 0.005, branch: 'crit', icon: '🎯' },
    critDamageBoost: { name: "Brutální Síla", description: "Zvyšuje poškození kritickým zásahem o 10% (multiplikativně) za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 2 + level, effectType: 'crit_damage_multiplier_bonus_percent', effectValue: 0.10, branch: 'crit', requires: 'critChanceBoost', icon: '💥' },
    ultimateCritMastery: { name: "Smrtící Přesnost", description: "Každých 20 kliknutí je zaručený kritický zásah.", maxLevel: 1, currentLevel:0, cost: () => 10, effectType: 'guaranteed_crit_every_x_hits', effectValue: 20, branch: 'crit', requires: 'critDamageBoost', isUltimate: true, icon: '☠️' },

    // Pasivní DPS větev
    passivePercentFlatBoostTalent: { name: "Trvalý Oheň (%HP)", description: "Přidává +0.01% max HP/s k pasivnímu poškození za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 2 + level, effectType: 'passive_percent_flat_boost_talent', effectValue: 0.0001, branch: 'passive_dps', icon: '🔥' },
    passivePercentMultiplierTalent: { name: "Síla Aury (%HP)", description: "Zvyšuje veškeré pasivní poškození (%HP/s) o 5% za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 3 + level, effectType: 'all_passive_percent_multiplier_talent', effectValue: 0.05, branch: 'passive_dps', requires: 'passivePercentFlatBoostTalent', icon: '✨' },
    ultimateAuraOfDecay: { name: "Aura Rozkladu", description: "Nepřátelé ztrácí 0.5% svého MAX. zdraví za sekundu (max. 500 DPS).", maxLevel: 1, currentLevel:0, cost: () => 15, effectType: 'aura_percent_health_damage', effectValue: { percent: 0.005, cap: 500 }, branch: 'passive_dps', requires: 'passivePercentMultiplierTalent', isUltimate: true, icon: '💀' },

    // Větev Mistrovství Výprav
    expeditionSpeed: { name: "Zrychlené Plánování", description: "Zkracuje dobu trvání výprav o 2% za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 3 + level, effectType: 'expedition_duration_reduction_percent', effectValue: 0.02, branch: 'expedition_mastery', icon: '⏱️', requires: 'echoAffinity' },
    lootFinder: { name: "Hledač Pokladů", description: "Zvyšuje šanci na získání dodatečné odměny z výprav o 1% za úroveň.", maxLevel: 5, currentLevel:0, cost: (level) => 5 + level * 2, effectType: 'expedition_extra_reward_chance_percent', effectValue: 0.01, branch: 'expedition_mastery', requires: 'expeditionSpeed', icon: '💎' },
    efficientLogistics: { name: "Efektivní Logistika", description: "Snižuje cenu zahájení výprav (zlato i EÚ) o 2% za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 4 + level, effectType: 'expedition_cost_reduction_percent', effectValue: 0.02, branch: 'expedition_mastery', requires: 'expeditionSpeed', icon: '🚚' },
    expeditionSlotsTalent: { name: "Velitel Výprav", description: "Zvyšuje maximální počet souběžných výprav o 1.", maxLevel: 2, currentLevel:0, cost: (level) => 10 + (level * 5), effectType: 'expedition_slots_additive', effectValue: 1, branch: 'expedition_mastery', requires: 'lootFinder', isUltimate: false, icon: '➕' },

    // Větev Časové Ozvěny (Offline Progres)
    offlineGoldBonus: { name: "Ozvěny Hojnosti", description: "Zvyšuje procento získaného zlata offline o 5% (absolutní bonus k základní míře zisku) za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 3 + level, effectType: 'offline_gold_earn_percentage_additive_talent', effectValue: 0.05, branch: 'temporal_echoes', icon: '⏳', requires: 'echoAffinity'},
    offlineXpBonus: { name: "Ozvěny Vědění", description: "Zvyšuje procento získaných XP offline o 2% (absolutní bonus k základní míře zisku) za úroveň.", maxLevel: 10, currentLevel:0, cost: (level) => 4 + level, effectType: 'offline_xp_earn_percentage_additive_talent', effectValue: 0.02, branch: 'temporal_echoes', requires: 'offlineGoldBonus', icon: '🧠' },
    extendedOfflineTime: { name: "Protažené Spojení", description: "Zvyšuje maximální dobu započítávaného offline progresu o 1 hodinu za úroveň.", maxLevel: 4, currentLevel:0, cost: (level) => 7 + level * 3, effectType: 'max_offline_time_increase_hours_additive', effectValue: 1, branch: 'temporal_echoes', requires: 'offlineXpBonus', icon: '🔗' }
};
